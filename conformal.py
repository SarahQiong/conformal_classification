...
def gcq(scores,
    ...
    if tau == 1.0:
        sizes[:] = cumsum.shape[
            1]  # always predict max size if alpha==0. (Avoids numerical error.)
...
# Get the 'p-value'
def get_tau(score, target, I, ordered, cumsum, penalty, randomized,
            allow_zero_sets):  # For one example
    ...
    if idx == (0, 0):
        ...
        return U * ordered[idx] + cumsum[
            (idx[0], idx[1] - 1)] + (penalty[0:(idx[1][0] + 1)]).sum()
...
def pick_lamda_size(model, paramtune_loader, alpha, kreg, randomized,
    ...
    best_size = iter(paramtune_loader).__next__()[0][1].shape[
        0]  # number of classes
    ...
    for temp_lam in [0.001, 0.01, 0.1, 0.2, 0.5
        ...
        conformal_model = ConformalModelLogits(model,
                                               ...
                                               allow_zero_sets=allow_zero_sets,
                                               naive=False)
        top1_avg, top5_avg, cvg_avg, sz_avg = validate(paramtune_loader,
                                                       conformal_model,
                                                       print_bool=False)
        if sz_avg < best_size:
            best_size = sz_avg
...
def pick_lamda_adaptiveness(model,
                            paramtune_loader,
                            alpha,
                            kreg,
                            randomized,
    ...
    for temp_lam in [0, 1e-5, 1e-4, 8e-4, 9e-4, 1e-3, 1.5e-3, 2e-3
        ...
        conformal_model = ConformalModelLogits(model,
                                               ...
                                               allow_zero_sets=allow_zero_sets,
                                               naive=False)
        curr_violation = get_violation(conformal_model, paramtune_loader,
                                       strata, alpha)
...
def pick_parameters(model, calib_logits, alpha, kreg, lamda, randomized,
                    allow_zero_sets, pct_paramtune, batch_size,
                    lamda_criterion):
    ...
    calib_loader = tdata.DataLoader(calib_logits,
                                    ...
                                    shuffle=False,
                                    pin_memory=True)
    paramtune_loader = tdata.DataLoader(paramtune_logits,
                                        batch_size=batch_size,
                                        shuffle=False,
                                        pin_memory=True)
    ...
    if lamda == None:
        if lamda_criterion == "size":
            ...
            lamda = pick_lamda_adaptiveness(model, paramtune_loader, alpha,
                                            kreg, randomized, allow_zero_sets)
...
def conformal_calibration_all(cmodel, calib_loader):
    ...
    Qhat['optimal_o'] = compute_overall_qhat(scores,
                                             targets,
                                             "optimal",
                                             np.zeros_like(cmodel.penalties),
                                             cmodel.alpha,
                                             class_specific=False)
